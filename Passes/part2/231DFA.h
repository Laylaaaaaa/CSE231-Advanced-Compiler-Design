...
class DataFlowAnalysis {
  ...
  public:
    ...
    void runWorklistAlgorithm(Function * func) {
    	...
    	for(auto it = InstrToIndex.begin(); it != InstrToIndex.end(); it++){
		...
		while(!worklist.empty()) {
    		...
    		unsigned Instr_Index = worklist.front();
    		worklist.pop_front();

    		if (Instr_Index == 0) continue;
    		Instruction * Instr = IndexToInstr[Instr_Index];

    		std::vector<unsigned> IncomingEdges;
	  		std::vector<unsigned> OutgoingEdges;
	  		std::vector<Info *> Flow_Result;

	  		getIncomingEdges(Instr_Index, &IncomingEdges);
	  		getOutgoingEdges(Instr_Index, &OutgoingEdges);

	  		flowfunction(Instr, IncomingEdges, OutgoingEdges, Flow_Result);

	  		for(unsigned i=0; i < OutgoingEdges.size(); i++) {

	  			Edge OutEdge = std::make_pair(Instr_Index, OutgoingEdges[i]);
	  			Info * newInfo = new Info();

	  			Info::join(Flow_Result[i], EdgeToInfo[OutEdge], newInfo);
